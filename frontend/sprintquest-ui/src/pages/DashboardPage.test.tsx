...
describe('DashboardPage', () => {
  ...
  it('loads and shows sprint progress and gamification metrics', () => {
    ...
    expect(
      screen.getByText('First Task Complete'),
    ).toBeInTheDocument();

    expect(
      screen.getByText('Sprint Starter'),
    ).toBeInTheDocument();
  });

  it('shows a loading state while dashboard metrics are loading', () => {
    useAppSelectionStore.setState({
      selectedProjectId: 'project-1',
      selectedSprintId: 'sprint-1',
    });
    ...
    expect(
      screen.getByRole('status', {
        name: /loading dashboard metrics/i,
      }),
    ).toBeInTheDocument();
  ...
  it('shows an error when sprint tasks fail to load', () => {
    useAppSelectionStore.setState({
      selectedProjectId: 'project-1',
      selectedSprintId: 'sprint-1',
    });
    ...
    useBoardStore.setState({
      isLoading: false,
      errorMessage: 'Unable to load sprint tasks.',
    });
